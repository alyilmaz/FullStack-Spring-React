...
test("initial button status", () =>{
    render(<Pagination page={0} count={15} rowsPerPage={5} totalPages={3} setRowsPerPage={p =>console.log(p)} setPage={p =>console.log(p)}/>);
...
test("check the buttons status at end of the page", () =>{
    render(<Pagination page={2} count={15} rowsPerPage={5} totalPages={3} setRowsPerPage={p =>console.log(p)} setPage={p =>console.log(p)}/>);
...
test("button background color check at the middle", () =>{
    render(<Pagination page={1} count={15} rowsPerPage={5} totalPages={3} setRowsPerPage={p =>console.log(p)} setPage={p =>console.log(p)}/>);
    expect(screen.getByTestId("button-first")).toHaveStyle(`color: ButtonText`);
    expect(screen.getByTestId("button-prev")).toHaveStyle(`color: ButtonText`);
    expect(screen.getByTestId("button-next")).toHaveStyle(`color: ButtonText`);
    expect(screen.getByTestId("button-last")).toHaveStyle(`color: ButtonText`);
...
test("check the value of page and rows data", () =>{
    render(<Pagination page={0} count={15} rowsPerPage={5} totalPages={3} setRowsPerPage={p =>console.log(p)} setPage={p =>console.log(p)}/>);
    let docPage = document.getElementById("text-page") as HTMLFormElement;
    let docRow = document.getElementById("text-rows") as HTMLFormElement;
    expect(docPage.textContent).toEqual("Page 1 of 3");
    expect(docRow.textContent).toEqual("Rows: 1 - 5 of 15");
})

test("check the value of page and rows data at another page", () =>{
    render(<Pagination page={1} count={17} rowsPerPage={4} totalPages={5} setRowsPerPage={p =>console.log(p)} setPage={p =>console.log(p)}/>);
    let docPage = document.getElementById("text-page") as HTMLFormElement;
    let docRow = document.getElementById("text-rows") as HTMLFormElement;
    expect(docPage.textContent).toEqual("Page 2 of 5");
    expect(docRow.textContent).toEqual("Rows: 5 - 8 of 17");
})
...
test("create snapshot", () =>{
    const tree = renderer
                    .create(<Pagination page={1} count={17} rowsPerPage={4} totalPages={5} setRowsPerPage={p =>console.log(p)} setPage={p =>console.log(p)}/>)
                    .toJSON();
