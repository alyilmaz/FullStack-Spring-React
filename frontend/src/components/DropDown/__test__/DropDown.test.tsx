...
test("status option place holder should be options", () =>{
    ...
    render(<DropDown options={options} onChange = {e => console.log(e)}/>);
    expect(screen.getByText("options")).toBeTruthy();
})

test("status option place holder should be same the name of placeholder of props", () =>{
    const placeholder = "test";
    render(<DropDown placeholder={placeholder} options={options} onChange = {e => console.log(e)}/>);
...
test("there is only one option in select element if no options pros", () =>{
    const placeholder = "test";
    render(<DropDown placeholder={placeholder} onChange = {e => console.log(e)}/>);
    let doc = document.getElementById('select')  as HTMLFormElement;
    expect(doc.childElementCount).toEqual(1);
})
...
test("the number of total options in select element should be size of options array(props) + 1", () =>{
    ...
    render(<DropDown placeholder={placeholder} options={options} onChange = {e => console.log(e)}/>);
    let doc = document.getElementById('select') as HTMLFormElement;
    expect(doc.childElementCount).toEqual(totatlChildElement);
})

test("all opitons should be enabled except status", () =>{
 
    render(<DropDown placeholder={"Status"} options={options} onChange = {e => console.log(e)}/>);
...
test("create snaphot", () =>{
    const tree = renderer
                    .create(<DropDown placeholder={"Status"} options={options} onChange = {e => console.log(e)}/>)
                    .toJSON();
